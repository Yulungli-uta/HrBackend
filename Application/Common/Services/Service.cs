...
public class Service<TEntity, TKey> : IService<TEntity, TKey> where TEntity : class
{
    ...
    public async Task UpdateAsync(TKey id, TEntity entity, CancellationToken ct) { await _repo.UpdateAsync(id,entity, ct); }
    public async Task UpdateAsync1(TKey id, TEntity entity, CancellationToken ct) { await _repo.UpdateAsync1(entity, ct); }
