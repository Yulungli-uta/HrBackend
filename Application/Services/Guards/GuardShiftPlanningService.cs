        ...
        foreach (var group in groups)
        {
            ...
            for (var date = dto.StartDate; date <= dto.EndDate; date = date.AddDays(1))
            {
                ...
                foreach (var emp in activeEmployees)
                {
                    ...
                    // Horario(s) efectivo(s): FixedScheduleId de condición especial sobreescribe
                    // TODOS los horarios paralelos del día (el empleado no se duplica por cada uno).
                    var effectiveScheduleIds = rule?.FixedScheduleId is int fixedScheduleId
                        ? new List<int> { fixedScheduleId }
                        : dayScheduleIds;
