...
public class GuardShiftPlanningService : IGuardShiftPlanningService
{
    ...
    public async Task<GuardShiftPlanningResultDto> CreateRecurringAsync(CreateRecurringGuardShiftPlanningDto dto, CancellationToken ct)
    {
        ...
        {
            var workDate = dto.StartDate.AddDays(7 * week);
            try
            {
                var validateReq = new ValidateGuardAssignmentRequestDto(
                    dto.EmployeeId, dto.LocationId, workDate, dto.ScheduleId, null, false);
                var validation = await _validationService.ValidateAsync(validateReq, ct);

                if (validation.HasBlockingErrors)
                {
                    skipped++;
                    var blocking = validation.Validations.Where(v => v.Severity == "BLOCKING").Select(v => v.Message);
                    messages.Add($"  {workDate:dd/MM/yyyy}: omitido — {string.Join("; ", blocking)}");
                    continue;
                }
            ...
            {
                errors++;
                messages.Add($"  {workDate:dd/MM/yyyy}: error — {ex.Message}");
            }
        }

        messages.Insert(0, $"Asignación recurrente: {generated} de {dto.RepeatWeeks} semana(s) creadas, {skipped} omitidas por conflicto, {errors} errores.");
    ...
    public async Task<GuardShiftPlanningResultDto> GenerateAsync(GenerateGuardShiftPlanningRequestDto dto, CancellationToken ct)
    {
        ...
        if (!group.IsActive)
            throw new InvalidOperationException($"El grupo {dto.GroupId} está inactivo, no se puede generar planificación para él.");
        ...
        for (var date = dto.StartDate; date <= dto.EndDate; date = date.AddDays(1))
        {
            ...
            // Un día del ciclo puede tener más de un horario en paralelo (ver RunGenerationLogicAsync);
            // todo el grupo activo trabaja cada uno de esos horarios ese día.
            var patternDetails = pattern.Details.Where(d => d.DayOrder == cycleDay + 1).ToList();
            if (patternDetails.Count == 0 || patternDetails.Any(d => d.IsRestDay)) continue;

            foreach (var patternDetail in patternDetails)
            {
        ...
        if (p.GroupId.HasValue)
        {
            ...
            if (groupPattern is not null)
            {
                ...
                if (pattern is not null)
                {
                    ...
                    // Días con más de un horario en paralelo se muestran unidos con "+" (ej. "M+N")
                    // para no confundirlos con dos días distintos de la secuencia.
                    patternSequence = string.Join("-", pattern.Details
                        .GroupBy(d => d.DayOrder)
                        .OrderBy(g => g.Key)
                        .Select(g => g.First().IsRestDay
                            ? "L"
                            : string.Join("+", g.Select(d => d.Schedule?.ScheduleCode?.Substring(0, 1) ?? "?"))));
                }
            }
        }
        ...
        GeneratePreviewRequestDto dto, bool dryRun, CancellationToken ct)
    {
        ...
        // Empleado/fecha ya cubierto por OTRO grupo dentro de este mismo lote (ej. empleado
        // activo en 2 grupos cuyos patrones coinciden en turno real el mismo día). Se registra
        // aquí (no solo en BD) para detectarlo en el mismo ciclo — incluso en dry run — en vez
        // de que ambos grupos pasen la validación y choquen recién al SaveChanges final.
        var claimedByGroup = new Dictionary<(int EmployeeId, DateOnly WorkDate), (int GroupId, string GroupName)>();
        ...
        foreach (var group in groups)
        {
            ...
            for (var date = dto.StartDate; date <= dto.EndDate; date = date.AddDays(1))
            {
                ...
                // Un día del ciclo puede tener más de un horario en paralelo (ej. sábado mañana +
                // sábado velada) — todo el grupo activo trabaja cada uno de esos horarios ese día,
                // salvo que tenga un FixedScheduleId de condición especial (ver más abajo).
                var patternDetails = pattern.Details.Where(d => d.DayOrder == cycleDay).ToList();

                if (patternDetails.Count == 0 || patternDetails.Any(d => d.IsRestDay))
                {
                ...
                var dayScheduleIds = patternDetails.Where(d => d.ScheduleId.HasValue).Select(d => d.ScheduleId!.Value).ToList();
                if (dayScheduleIds.Count == 0) continue;
                ...
                foreach (var emp in activeEmployees)
                {
                    ...
                    // Si la condición especial fija un horario que YA es uno de los del patrón
                    // ese día, no hay conflicto real (coincide). Si fija uno DISTINTO, no se
                    // aplica en silencio — se reporta como conflicto para que el responsable lo
                    // revise (así se generó el bug real de guardias con el horario de su grupo
                    // anterior tras moverlos de grupo: la regla especial vieja quedó vigente).
                    if (rule?.FixedScheduleId is int fixedScheduleId && !dayScheduleIds.Contains(fixedScheduleId))
                    {
                        scheduleMap.TryGetValue(fixedScheduleId, out var fixedSchedule);
                        var patternScheduleLabels = dayScheduleIds
                            .Select(id => scheduleMap.TryGetValue(id, out var s) ? (s.ScheduleCode ?? s.Description) : id.ToString());
                        items.Add(MakePreviewItem(
                            emp, group, null, null, date,
                            fixedScheduleId, fixedSchedule?.ScheduleCode, fixedSchedule?.Description ?? "",
                            cycleDay, false, true, "SPECIAL_RULE_OVERRIDE",
                            $"Tiene una regla especial fija a '{fixedSchedule?.ScheduleCode ?? fixedSchedule?.Description}', pero el patrón de {group.Name} le asigna '{string.Join("/", patternScheduleLabels)}' este día. Revisa o actualiza la regla especial antes de generar.",
                            false, false));
                        continue;
                    }

                    var effectiveScheduleIds = dayScheduleIds;

                    foreach (var effectiveScheduleId in effectiveScheduleIds)
                    {
                        ...
                        var batchKey = (emp.EmployeeId, date);
                        // Solo es un conflicto real cuando el turno ya quedó cubierto por OTRO
                        // grupo. Si es el mismo grupo (ej. patrón especial con jornada mañana+
                        // noche el mismo día, como Celeste), no es solapamiento — es el múltiple
                        // horario normal de ese grupo, ya gobernado por AllowDoubleShift.
                        if (claimedByGroup.TryGetValue(batchKey, out var claimedBy) && claimedBy.GroupId != group.GroupId)
                        {
                            items.Add(MakePreviewItem(
                                emp, group, resolvedLocationId, resolvedLocationName, date,
                                effectiveScheduleId, schedule?.ScheduleCode, schedule?.Description ?? "",
                                cycleDay, false, true, "GROUP_OVERLAP",
                                $"Ya tiene turno asignado este día por el grupo '{claimedBy.GroupName}'. También pertenece a '{group.Name}', que intenta asignarle otro turno el mismo día — revisa si la membresía en ambos grupos es correcta.",
                                false, false));
                            continue;
                        }
                        ...
                        if (!willSkip && !hasConflict)
                        {
                            claimedByGroup[batchKey] = (group.GroupId, group.Name);
    ...
    private static GeneratePreviewResponseDto BuildPreviewResponse(List<GeneratePreviewItemDto> items, int restDays) =>
        new(
            ...
            items.Count(i => i.ConflictType == "SPECIAL_RULE_OVERRIDE"),
            items
        );
