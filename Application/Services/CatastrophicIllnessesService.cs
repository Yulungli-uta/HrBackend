...
{
    // Mismo directorio compartido de toda la hoja de vida (ver EducationLevelsService) — el
    // expediente de cada persona queda agrupado por identificación en vez de disperso por módulo.
    private const string DirectoryCode = "HR_HOJA_DE_VIDA";
    private const string EntityTypeName = "CATASTROPHIC_ILLNESS";

    private readonly ICatastrophicIllnessesRepository _repository;
    ...
    public CatastrophicIllnessesService(
        ICatastrophicIllnessesRepository repo,
    ...
    public async Task<(CatastrophicIllnesses entity, StoredFile? storedFile, string? error)> CreateWithDocumentAsync(
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            var (created, storedFile) = await _txRepo.CreateWithDocumentAsync(
                entity,
                physical != null
                    ? (e => StoredFileBuilder.Build(DirectoryCode, EntityTypeName, e.IllnessId.ToString(), file!, physical, documentTypeId))
                    : null,
                ct);
