...
public class LanguagesService : Service<Languages, int>, ILanguagesService
{
    ...
    private const string EntityTypeName = "LANGUAGE";
    ...
    public async Task<(Languages entity, StoredFile? storedFile, string? error)> CreateWithDocumentAsync(
        Languages entity,
        IFormFile? file,
        int? documentTypeId,
        CancellationToken ct)
    {
        ...
        try
        {
            var (created, storedFile) = await _txRepo.CreateWithDocumentAsync(
                entity,
                physical != null
                    ? (e => StoredFileBuilder.Build(DirectoryCode, EntityTypeName, e.LanguageId.ToString(), file!, physical, documentTypeId))
                    : null,
                ct);
